...
const JogWheel: React.FC<JogWheelProps> = ({ id, playing }) => {
  const [rotation, setRotation] = useState(0);
  const accentColor = id === DeckId.A ? 'border-deckA shadow-glow-A' : 'border-deckB shadow-glow-B';
  const indicatorColor = id === DeckId.A ? 'bg-deckA' : 'bg-deckB';
